...
cidades = [(random.randint(0, 50), random.randint(0, 50)) for _ in range(num_cidades)]

# Medir o tempo de execução
start_time = time.time()

# Encontrar o menor caminho usando força bruta
melhor_caminho, menor_distancia, num_execucoes = menor_caminho_forca_bruta(cidades)
...
for i in range(len(cidades)):
    for j in range(i + 1, len(cidades)):
        plt.plot([x_cidades[i], x_cidades[j]], [y_cidades[i], y_cidades[j]], "k-", linewidth=0.5, alpha=0.3)

for i in range(len(melhor_caminho) - 1):
    plt.plot([x_cidades[melhor_caminho[i]], x_cidades[melhor_caminho[i + 1]]], [y_cidades[melhor_caminho[i]], y_cidades[melhor_caminho[i + 1]]], "r-", linewidth=2)
